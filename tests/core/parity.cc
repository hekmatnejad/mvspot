...
static int kind2num(spot::parity_kind kind)
{
  switch (kind)
    {
      case spot::parity_kind_min:
        return 0;
      case spot::parity_kind_max:
        return 1;
      case spot::parity_kind_same:
        return 2;
      case spot::parity_kind_any:
        return 3;
    }
  return 4;
}
  ...
  if (!(is_max == target_max && is_odd == target_odd))
    {
      ...
      std::cerr << "target: " << kind[kind2num(target_kind)] << ' '
