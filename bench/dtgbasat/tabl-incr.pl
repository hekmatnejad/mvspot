...
print "\\begin{tabular}{lrcr|r|rrrr|rrr|rr|rrr|rrr|rrrr|rrr|rrr|rrrr|rrr|rrr|rrrr}";
print "\\multicolumn{44}{l}{Column \\textbf{type} shows how the initial det. aut. was obtained: T = translation produces DTGBA; W = WDBA minimization works; P = powerset construction transforms TBA to DTBA; R = DRA to DBA.}\\\\\n";
print "\\multicolumn{44}{l}{Column \\textbf{C.} tells whether the output automaton is complete: rejecting sink states are always omitted (add 1 state when C=0 if you want the size of the complete automaton).}\\\\\n";
print "\\multicolumn{14}{|c}{}& \\multicolumn{10}{|c}{param = 1}& \\multicolumn{10}{|c}{param = 2}& \\multicolumn{10}{|c}{param = 3}\\\\\n";
print "&&&&DRA&\\multicolumn{4}{|c}{DTGBA} & \\multicolumn{3}{|c}{DBA} &  \\multicolumn{2}{|c}{DBA\\footnotesize minimizer}&  \\multicolumn{3}{|c}{min DBA} & \\multicolumn{3}{|c}{minDTBA} & \\multicolumn{4}{|c}{minDTGBA} & \\multicolumn{3}{|c}{min DBA} & \\multicolumn{3}{|c}{minDTBA} & \\multicolumn{4}{|c}{minDTGBA} &  \\multicolumn{3}{|c}{min DBA} & \\multicolumn{3}{|c}{minDTBA} & \\multicolumn{4}{|c}{minDTGBA}\\\\\n";
print "formula& \$m\$ & type & C. & st.& st. & tr. & acc. & time & st. & tr. & time & st. & time & st. & tr. & time & st. & tr. & time & st. & tr. & acc. & time & st. & tr. & time & st. & tr. & time & st. & tr. & acc. & time & st. & tr. & time & st. & tr. & time & st. & tr. & acc. & time\\\\\n";
...
foreach my $tab (@w)
{
    ...
    print "{\\P}" if val(8) == 0 || val(17) == 0 || val(26) == 0 || val(35) == 0  || val(44) == 0 || val(53) == 0 || val(62) == 0 || val(71) == 0 || val(80) == 0 || val(89) == 0 || val(98) == 0 || val(10) == 0;
    print "$tab->[9] & ";

    if ($tab->[105] =~ m:\s*n/a\s*:) #  DRA
    {
	print "&";
	$tab->[105] = 0+'inf';
    }
    ...
    if ($tab->[102] =~ /\s*-\s*/) #  DBAminimizer
    {
	print "\\multicolumn{2}{c|}{(killed)}&";
	$tab->[102] = 0+'inf';
    }
    elsif ($tab->[102] =~ m:\s*n/a\s*:) #  DBAminimizer
    {
	print " & &";
	$tab->[102] = 0+'inf';
    }
