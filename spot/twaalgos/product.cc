          ...
          for (auto& first_edge: (*begin)->out(*top.first.begin()))
            {
              ...
              std::function<void(bdd, acc_cond::mark_t, unsigned,
                                 product_n_state, I,
                                 std::vector<unsigned>::iterator)> traversal =
                [&](bdd cond, acc_cond::mark_t acc, unsigned current_state,
                  ...
                  for (auto& edge: cur_edges)
                    {
                      ...
                      if (cond == bddfalse)
                        continue;
                      acc &= edge.acc << *current_acc_shift;
                      dst.push_back(edge.dst);
                      if (std::next(current_automaton) == end)
                      {
                        auto dst_state = new_state(dst);
                        res->new_edge(top.second, dst_state, cond, acc);
                      }
                      else
                        traversal(cond, acc, edge.dst, dst,
                                  std::next(current_automaton),
                                  std::next(current_acc_shift));
                  }
                };
              traversal(cond, acc, *(top.first.begin()), dst, std::next(begin),
                        acc_shift.begin());
            }
        }
