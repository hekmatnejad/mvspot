...
namespace spot
{
  namespace
  {
    ...
    struct dict
    {
          ...
          unsigned dst_ref)
      {
        ...
        catch (const std::out_of_range& c)
        {
          std::cerr << "path(" << src_ref << ',' << dst_ref
            << ") of pathid_ref " << fmt_p(src_cand, src_ref, dst_cand, dst_ref)
            << " not found.\n";
          throw c;
        }
#else
          ...
          unsigned dst_ref)
      {
        ...
        {
          std::cerr << "path(" << src_ref << ',' << dst_ref
            << ") of pathid_can " << fmt_p(src_cand, src_ref, dst_cand, dst_ref)
            << " not found.\n";
          throw c;
        }
#else
                    ...
                    for (unsigned q2 = 0; q2 < d.cand_size; q2++)
                      {
                        ...
                        cnf_comment(prev, " ∧ ", d.fmt_t(q1, s, q2),
                            "δ → ", d.fmt_p(q2, dp, q2, dp), '\n');
  ...
  static twa_graph_ptr
  dichotomy_dtba_research(int max, dict& d, satsolver& solver,
      ...
      bool state_based)
  {
    ...
    trace << "End of dichotomy: max<" << max << ">, min<" << min << ">\n";
    if (!res)
    {
    ...
    t1.stop("solve");
    print_log(t1, d.cand_size - target, res, solver); // If SPOT_SATLOG is set.
    return res ? res : next;
  }
      ...
      bool state_based, int max_states, unsigned param)
  {
    ...
    do
    {
      ...
      if (solution.second.empty()) // UNSAT
      {
        ...
        return dichotomy_dtba_research(nassume_count, d, solver, t1, prev,
            next, state_based);
      }
      ...
      bool state_based, int max_states, int sat_swap)
  {
    if (!a->acc().is_buchi())
      throw std::runtime_error
        ("dtba_sat_minimize_incr2() can only work with Büchi acceptance");
    const_twa_graph_ptr prev = a;
    dict d;
    d.cand_size = (max_states < 0) ?
      stats_reachable(prev).states - 1 : max_states;
    ...
    trace << "dtba_sat_minimize_incr2(..., states = " << d.cand_size
      << ", state_based = " << state_based << ")\n";
